...
#### there were some problems with 3 samples called (P1, P2, ecc). Carlos combined them in a separate scripts using the commands at the end of the script
for (i in seq_along(metadata$SampleName)) {
  if (!file.exists(file.path(qc_data, 'individual', sprintf("%s-cm_list.rds", metadata$SampleName[i]))) &
      !file.exists(file.path(qc_data, 'individual', sprintf("%s-qc_raw_data.rds", metadata$SampleName[i]))) &
      !file.exists(file.path(qc_data, 'individual', sprintf("%s-qc_filtered_data.rds", metadata$SampleName[i]))) &
      !file.exists(file.path(qc_data, 'individual', sprintf("%s-samples.rds", metadata$SampleName[i]))) &
      !file.exists(file.path(qc_data, 'individual', sprintf("%s-genes_to_keep.rds", metadata$SampleName[i])))) {
    cat(cyan(sprintf("Sample %s\n", metadata$SampleName[i])))
    
    ## Load count matrix
    cm <- lapply(list.files(file.path(base_dir, "data/cm"), pattern = glue('cm_counts_{metadata$SampleName[i]}.csv.gz'), full.names = T), function(x) {
    ...
    ## Load alignment qc
    qc_align <- lapply(list.files(file.path(base_dir, "data/cm"), pattern = glue('qc_{metadata$SampleName[i]}.csv.gz'), full.names = T), function(x) {
      fread(x, data.table = F) %>% 
    ...
    # I modified the code, as there was the problem of "BT2126-P1" "BT2126-P2" "BT2126-P3" "BT2126-P4" being called with 4 names instead of 1
    samples <- structure(sub("-.*$", "", qc_align[,"sample"]), names = rownames(qc_align))
    ...
    p4 <- ggplot(qc_raw_data, aes(x=align_rate)) + geom_histogram(bins=50) +
      ...
      theme_classic() + theme(plot.title = element_text(hjust = 0.5, face="bold", size=14),
                              ...
                              legend.text = element_text(size=14))
    (p1+p2)/(p3+p4)
    ggsave(sprintf("%s-QC_histogram_nGene_nAligned.pdf", metadata$SampleName[i]), path = file.path(qc_plots, 'individual'), width = 12, height=8)
    ...
    if (metadata$Type[i] == 'frozen') {
      ...
      ggplot(data = qc_raw_data, aes(x=gene, y=align_rate, color=pass_qc)) + geom_point() +
        ...
        theme_classic() + theme(plot.title = element_text(hjust = 0.5, face="bold", size=16),
                                ...
                                legend.title = element_text(face="bold", size=16),
                                legend.text = element_text(size=14))
      ggsave(sprintf("%s-genes_vs_alignrate.pdf", metadata$SampleName[i]), path = file.path(qc_plots, 'individual'), width = 8, height=6)
      ...
      ggplot(data=qc_raw_data, aes(x=gene, y=hk, color=pass_qc)) + geom_point() +
        ...
        theme_classic() + theme(plot.title = element_text(hjust = 0.5, face="bold", size=16),
                                ...
                                legend.title = element_text(face="bold", size=16),
                                legend.text = element_text(size=14))
      ggsave(sprintf("%s-genes_vs_hk.pdf", metadata$SampleName[i]), path = file.path(qc_plots, 'individual'), width = 8, height=6)
    }
    ...
    saveRDS(cm_list, file = file.path(qc_data, 'individual', sprintf("%s-cm_list.rds", metadata$SampleName[i])))
    saveRDS(qc_raw_data, file = file.path(qc_data, 'individual', sprintf("%s-qc_raw_data.rds", metadata$SampleName[i])))
    saveRDS(qc_filtered_data, file = file.path(qc_data, 'individual', sprintf("%s-qc_filtered_data.rds", metadata$SampleName[i])))
    saveRDS(samples, file = file.path(qc_data, 'individual', sprintf("%s-samples.rds", metadata$SampleName[i])))
    saveRDS(genes_to_keep, file = file.path(qc_data, 'individual', sprintf("%s-genes_to_keep.rds", metadata$SampleName[i])))
  }
}
...
# sample <- 'BT1804Tumor'
# cm <- lapply(list.files(file.path(base_dir, "data/cm"), pattern = glue('cm_counts_{sample}'), full.names = T), function(x) {
#  fread(x, data.table = F) %>%
#    column_to_rownames("V1") %>%
#    rename_with(~ gsub('-', '\\.', .x))
# })
# cm <- do.call(cbind, cm)
# qc_align <- lapply(list.files(file.path(base_dir, "data/cm"), pattern = glue('qc_{sample}'), full.names = T), function(x) {
#  fread(x, data.table = F) %>%
#    column_to_rownames('name')
# })
# qc_align <- do.call(rbind, qc_align)
# qc_align <- qc_align[colnames(cm), ]
# qc_align <- qc_align %>% rownames_to_column('name')
# write.csv(cm, glue('/n/scratch/users/c/cao385/cm_counts_{sample}.csv'), quote = F)
# write.csv(qc_align, glue('/n/scratch/users/c/cao385/qc_{sample}.csv'), row.names = F, quote = F)
# 
# 
# sample <- 'BT2126'
# cm <- lapply(list.files(file.path(base_dir, "data/cm"), pattern = glue('cm_counts_{sample}'), full.names = T), function(x) {
