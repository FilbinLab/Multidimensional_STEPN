...
# load in data -----------------------------------
# seurat object
seurat_obj <- qread(file = file.path(base_dir, "data/patient/seurat_obj_ST_normal_malig_annotated.qs"))
...
DimPlot(object = seurat_obj, reduction = 'umap.harmony', group.by = "cnv", label = FALSE, pt.size = 0.5, shuffle = TRUE, label.size = 0, cols = c( 'red', 'black')) +
...
DimPlot(object = seurat_obj, reduction = 'umap.harmony', group.by = "Subtype", pt.size = 0.8, shuffle = TRUE,  label = TRUE,label.size = 0,cols = col_subtype) + 
  labs(title = 'ST-EPN frozen', subtitle = 'n = 7,429 cells') + theme_tSNE
...
ggsave(file.path(plot_dir, '5_sampleID_harmony.pdf'), width = 7.5, height = 5) 
...
ggsave(file.path(plot_dir, '6_sampleID_unintegrated.pdf'), width = 7.5, height = 5) 


## UMAP By classification used for NMF
...
plot <- FeaturePlot(seurat_obj,
            ...
            cols = c("lightgrey", "red"), ncol = 5, pt.size = 2, combine = F, raster = TRUE)
ggarrange(plotlist = plot, ncol = 6, nrow = 2)
ggsave(file.path(plot_dir, '10_UMAP_markers_harmony.pdf'), width = 18, height = 6) 
...
plot <- FeaturePlot(seurat_obj,
                  ...
                  cols = c("lightgrey", "red"), ncol = 5, pt.size = 2, combine = F, raster = TRUE)
ggarrange(plotlist = plot, ncol = 6, nrow = 2)
ggsave(file.path(plot_dir, '11_UMAP_markers_unintegrated.pdf'), width = 18, height = 6) 
...
DimPlot(object = seurat_obj, reduction = 'umap.harmony', group.by = "SingleR.labels", label = TRUE, 
  ...
  labs(title = 'SingleR prediction', subtitle = 'n = 7,429 cells') 
...
DimPlot(object = seurat_obj, reduction = 'umap.harmony', group.by = "tumor_manual", label = TRUE, cols = c( 'black', 'red'),
        pt.size = 0.5, raster = FALSE, shuffle = TRUE, label.size = 0) +  
  labs(title = 'inferCNV', subtitle = 'n = 7,429 cells') 
ggsave(file.path(plot_dir, '16_UMAP_Manual_classification_harmony.pdf'), width = 6, height = 5) 

DimPlot(object = seurat_obj, reduction = 'umap.unintegrated', group.by = "tumor_manual", label = TRUE, cols = c('black', 'red'),
        pt.size = 0.5, raster = FALSE, shuffle = TRUE, label.size = 0) + 
  labs(title = 'inferCNV', subtitle = 'n = 7,429 cells') 
ggsave(file.path(plot_dir, '17_UMAP_Manual_classification.pdf'), width = 6, height = 5) 

DimPlot(object = seurat_obj, reduction = 'umap.harmony', group.by = "cell_type", label = TRUE, cols = colors_tumor_calling,
