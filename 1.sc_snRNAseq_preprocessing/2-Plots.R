...
files <- sprintf(file.path(base_dir, 'analysis/qc/data/individual/%s-qc_raw_data.rds'), unique(metadata$SampleName))
...
h1 <- ggplot(qc) +
  ...
  geom_hline(yintercept = 0.15, color = "red", linetype = "dashed", linewidth = 0.5) +
...
ggsave(file.path(base_dir, 'analysis/qc/qc.pdf'), width = 20, height = 4)
