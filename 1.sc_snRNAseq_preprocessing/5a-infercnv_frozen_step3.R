...
infercnv_plots <- file.path(infercnv_dir, 'plots')
...
metadata <- data.frame(sample = samples, type = obs_ref, cnv = cnv_stat)
suppressWarnings(seurat_obj <- RunFullSeurat_v5(cm = cm, metadata = metadata,  doBatch = T, var2batch = 'sample', batchMethod = 'harmony', project = 'EPN'))
saveRDS(seurat_obj, file = file.path(infercnv_dir, "seurat_obj.rds"))

## By cluster
p1 <- DimPlot(object = seurat_obj, reduction = 'umap.harmony', group.by = 'harmony_clusters', label = TRUE, label.box = T, pt.size = 0.1, label.size = 2) +
  ggtitle('Clusters') +
  seurat_theme() +
  NoLegend()
...
#normal_cells <- c(myeloid_cells, oligo_cells, endothelial_cells, pericyte_cells) %>% unique


normal_cells <- Cells(seurat_obj)[seurat_obj$seurat_clusters  %in% c(16, 15, 13, 10)]
...
DimPlot(object = seurat_obj,
        ...
        group.by = "tumor", 
        pt.size = 0.1, 
        cols = c("lightgrey", gg_color_hue(2))) +
  ggtitle('tumor') +
  seurat_theme()
ggsave("tumor.pdf", path = infercnv_plots, width = 5, height = 4)
...
suppressWarningsseurat_obj <- RunFullSeurat_v5(cm = cm_malignant, metadata = metadata,  doBatch = T, var2batch = 'sample', batchMethod = 'harmony', project = 'EPN')
