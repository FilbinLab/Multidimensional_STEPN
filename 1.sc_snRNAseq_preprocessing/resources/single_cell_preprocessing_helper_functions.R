...
RunFullSeurat_v5 <- function(cm, metadata, do.scale = FALSE, do.center = TRUE, doBatch = F, var2batch = NULL, batchMethod = 'harmony', resolution = 1, dims = 20, pca_dims = 100, n.neighbors = 30, project, norm.type = 'RNA', verbose = FALSE){
  ...
  if (doBatch){
    ...
    if (norm.type == 'RNA') {
      ...
      seurat_obj <- seurat_obj %>%
        ...
        FindClusters(resolution = resolution, cluster.name = "unintegrated_clusters", verbose = verbose) %>%
        RunUMAP(reduction = "pca", dims = 1:dims, reduction.name = "umap.unintegrated", return.model = TRUE, verbose = verbose) %>%
        RunTSNE(dims = 1:dims, reduction = "pca", reduction.name = "tsne.unintegrated", num_threads = 8, check_duplicates = F, verbose = verbose)
    }
    ...
    if (norm.type == 'RNA') {
      ...
      seurat_obj <- seurat_obj %>%
        FindNeighbors(reduction = "pca", dims = 1:dims, verbose = verbose) %>%
        FindClusters(resolution = resolution, verbose = verbose) %>%
        RunUMAP(reduction = "pca", dims = 1:dims, return.model = TRUE, verbose = verbose) %>%
        RunTSNE(reduction = 'pca', dims = 1:dims, num_threads = 8, check_duplicates = F, verbose = verbose)
    }
  }
  return(seurat_obj)
