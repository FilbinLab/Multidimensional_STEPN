...
RunFullSeurat_v5 <- function(cm, metadata, do.scale = FALSE, do.center = TRUE, doBatch = F, var2batch = NULL, batchMethod = 'harmony', resolution = 1, dims = 20, pca_dims = 100, n.neighbors = 30, tsne_perplexity = 30, project, norm.type = 'RNA', verbose = TRUE){
...
projectDataNoScaling <- function(query_cm, query_degs, query_hvgs, query_query_metagene_order, 
                                 ...
                                 outFile, cell_type_order, fig_width, fig_height, min_size, max_size, min_color, max_color) {
  ...
  pairwise_cor <- calculatePairwiseCor(as.matrix(ref_cm_norm), query_cm_norm, input_genes)
  ...
  ## Trimming values lower then 0 and greater then 1
  pairwise_cor_long$normal_score = trimScores(normal_score_long$score, 1, 0)
  pairwise_cor_long$tumor_score = trimScores(tumor_score_long$score, 1, 0)
  ...
  pt <- ggplot(pairwise_cor_long, aes(x=Cell_type, y=Metaprogram)) + 
    ...
    #geom_point(aes(color=tumor_score, size=normal_score, alpha=r)) + scale_size_area(max_size=9) +
    paletteer::scale_colour_paletteer_c("viridis::mako", direction = -1) +
    labs(x = "Normal cell type",
         y = "Malignant metaprogram",
    ...
    scale_fill_gradient(limits = c(min_color, max_color)) 
...
## Project dataset in a query (dotplot)
projectData <- function(query_cm, query_degs, query_hvgs, query_query_metagene_order, 
                                 ref_cm, ref_degs, ref_hvgs, 
