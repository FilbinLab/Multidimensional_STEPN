...
seurat_obj <- subset(seurat_obj, idents = c( "ZFTA-Cluster 1", 'ZFTA-Cluster 3', "ZFTA-Cluster 2", 'ZFTA-Cluster 4'), invert = T)
...
plot_dir <- file.path(base_dir, "analysis/plots/pseudobulk/ZR_vs_YAP1")
if (!dir.exists(plot_dir)){dir.create(plot_dir, recursive = T)}
...
markers <- RunPrestoAll(
  ...
  min.pct = 0.15,
  verbose = TRUE)
write.csv(markers, file.path(plot_dir, "1_markers_ZR_vsYAP1.csv"))
  
  markers2 <- markers %>% 
    filter(cluster %in% c("ST-YAP1")) %>%
...
```{r volcano, fig.width =6, fig.height = 4, fig.align="center"}
 ...
 ggplot(markers2, aes(avg_log2FC, -log(p_val_adj,10))) +
   ...
   xlim(c(-6, 6)) +
   labs(color = 'Gene')
  ggsave(file.path(plot_dir, "2_Volcano_plot_YAP1_vs_ZFTARELA.pdf"), width=6, height=4)
...
write.csv(go_result_df, file.path(plot_dir, "3_enrichGO_YAP1.csv"))


go_result_df <- go_result_df |>
  dplyr::filter(p.adjust < 0.01)
write.csv(go_result_df, file.path(plot_dir, "4_enrichGO_YAP1_pval_01.csv"))
...
ggsave(file.path(plot_dir, "5_Dotplot_YAP1.pdf"), width=6, height=3)
```
...
write.csv(go_result_df_down, file.path(plot_dir, "6_enrichGO_ZFTARELA.csv"))


go_result_df_down <- go_result_df_down |>
  dplyr::filter(p.adjust < 0.01)
write.csv(go_result_df_down, file.path(plot_dir, "7_enrichGO_ZFTARELA_pval_01.csv"))
...
go_ora_results_down %>% filter(p.adjust < 0.05) %>%
  ...
  theme(panel.background = element_blank(),  # Remove background
        ...
        axis.text = element_text(color = "black"),  # Change axis text color
        axis.title.y = element_text(color = "black"))  # Change y-axis title color
ggsave(file.path(plot_dir, "8_Dotplot_ZFTARELA.pdf"), width=9, height=3)
...
mlist<-list(go_ora_results_down, go_ora_results)
names(mlist)<-c("ZFTA-RELA", "ST-YAP1")
