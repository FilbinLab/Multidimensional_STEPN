...
seurat_obj <- subset(seurat_obj, idents = c('ST-YAP1', 'ZFTA-Cluster 2', "ZFTA-Cluster 3", 'ZFTA-Cluster 4'), invert = T)
...
plot_dir <- file.path(base_dir, "analysis/plots/pseudobulk/Cluster1_vs_ZFTARELA_Canonical")
if (!dir.exists(plot_dir)){dir.create(plot_dir, recursive = T)}
...
markers <- RunPrestoAll(
  ...
  min.pct = 0.15,
  verbose = TRUE)
write.csv(markers, file.path(plot_dir, "1_markers_cluster1_vs_canonical.csv"))
  
  markers2 <- markers %>% 
    filter(cluster %in% c("ZFTA-Cluster 1")) %>%
...
```{r volcano, fig.width =6, fig.height = 4, fig.align="center"}
 ...
 ggplot(markers2, aes(avg_log2FC, -log(p_val_adj,10))) +
   ...
   xlim(c(-6, 6)) +
   labs(color = 'Gene')
  ggsave(file.path(plot_dir, "2_Volcano_plot_cluster1_vs_ZFTA.pdf"), width=6, height=4)
...
write.csv(go_result_df, file.path(plot_dir, "3_enrichGO_Cluster1.csv"))


go_result_df <- go_result_df |>
  dplyr::filter(p.adjust < 0.01)
write.csv(go_result_df, file.path(plot_dir, "4_enrichGO_Cluster1_pval_01.csv"))
...
ggsave(file.path(plot_dir, "5_Dotplot_Cluster1.pdf"), width=6, height=3)
```
...
names(mlist)<-c("ZFTA-RELA", "ZFTA-Cluster 1")
