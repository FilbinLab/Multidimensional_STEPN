...
seurat_obj <- subset(seurat_obj, idents = c('ST-YAP1', 'ZFTA-RELA', "ZFTA-Cluster 1", 'ZFTA-Cluster 3'), invert = T)
...
plot_dir <- file.path(base_dir, "analysis/plots/pseudobulk/Cluster2_vs_Cluster4")
if (!dir.exists(plot_dir)){dir.create(plot_dir, recursive = T)}
...
markers <- RunPrestoAll(
  ...
  min.pct = 0.15,
  verbose = TRUE)
write.csv(markers, file.path(plot_dir, "1_markers_cluster2_vs_cluster4.csv"))
...
```{r volcano, fig.width =6, fig.height = 4, fig.align="center"}
 ...
 ggplot(markers2, aes(avg_log2FC, -log(p_val_adj,10))) +
   ...
   xlim(c(-6, 6)) +
   labs(color = 'Gene')
  ggsave(file.path(plot_dir, "2_Volcano_plot_cluster4_vs_cluster2.pdf"), width=6, height=4)
...
write.csv(go_result_df, file.path(plot_dir, "3_enrichGO_cluster4.csv"))


go_result_df <- go_result_df |>
  dplyr::filter(p.adjust < 0.01)
write.csv(go_result_df, file.path(plot_dir, "4_enrichGO_Cluster4_pval_01.csv"))
...
ggsave(file.path(plot_dir, "5_Dotplot_Cluster4.pdf"), width=6, height=3)
```
...
write.csv(go_result_df_down, file.path(plot_dir, "6_enrichGO_cluster2.csv"))


go_result_df_down <- go_result_df_down |>
  dplyr::filter(p.adjust < 0.01)
write.csv(go_result_df_down, file.path(plot_dir, "7_enrichGO_cluster2_pval_01.csv"))
...
go_ora_results_down %>% filter(p.adjust < 0.05) %>%
  ...
  theme(panel.background = element_blank(),  # Remove background
        ...
        axis.text = element_text(color = "black"),  # Change axis text color
        axis.title.y = element_text(color = "black"))  # Change y-axis title color
ggsave(file.path(plot_dir, "8_Dotplot_cluster2.pdf"), width=9, height=3)
