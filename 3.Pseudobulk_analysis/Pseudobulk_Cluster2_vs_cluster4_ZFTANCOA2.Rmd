...
seurat_obj <- subset(seurat_obj, idents = c( "ZFTA-Cluster 1", 'ZFTA-Cluster 3', "ST-YAP1", 'ZFTA-RELA'), invert = T)

Idents(seurat_obj) <- 'Fusion'
seurat_obj <- subset(seurat_obj, idents = c( "ZFTA::NCOA2"), invert = F)
...
plot_dir <- file.path(base_dir, "analysis/plots/pseudobulk/Cluster2_vs_Cluster4_NCOA1")
if (!dir.exists(plot_dir)){dir.create(plot_dir, recursive = T)}
...
markers <- RunPrestoAll(
  ...
  min.pct = 0.15,
  verbose = TRUE)
write.csv(markers, file.path(plot_dir, "1_markers_Cl4_vs_cl2.csv"))
  
  markers2 <- markers %>% 
    filter(cluster %in% c("ZFTA-Cluster 4")) %>%
...
```{r volcano, fig.width =6, fig.height = 4, fig.align="center"}
 ...
 ggplot(markers2, aes(avg_log2FC, -log(p_val_adj,10))) +
   ...
   xlim(c(-6, 6)) +
   labs(color = 'Gene')
  ggsave(file.path(plot_dir, "2_Volcano_plot_Cl4_vs_cl2.pdf"), width=6, height=4)
...
write.csv(go_result_df, file.path(plot_dir, "3_enrichGO_Cl4_vs_cl2.csv"))


go_result_df <- go_result_df |>
  dplyr::filter(p.adjust < 0.01)
write.csv(go_result_df, file.path(plot_dir, "4_enrichGO_Cl4_vs_cl2_pval_01.csv"))
...
go_ora_results %>% filter(p.adjust < 0.05) %>%
  ...
  xlab("Gene Ratio") +
  ylab(NULL) + 
  ggtitle("GO Enrichment of Cluster 4")+
...
ggsave(file.path(plot_dir, "5_Dotplot_Cl4_vs_cl2.pdf"), width=6, height=3)
```
...
write.csv(go_result_df_down, file.path(plot_dir, "6_enrichGO_Cl2.csv"))


go_result_df_down <- go_result_df_down |>
  dplyr::filter(p.adjust < 0.01)
write.csv(go_result_df_down, file.path(plot_dir, "7_enrichGO_Cl2_pval_01.csv"))
...
go_ora_results_down %>% filter(p.adjust < 0.05) %>%
  ...
  theme(panel.background = element_blank(),  # Remove background
        ...
        axis.text = element_text(color = "black"),  # Change axis text color
        axis.title.y = element_text(color = "black"))  # Change y-axis title color
ggsave(file.path(plot_dir, "8_Dotplot_Cl2.pdf"), width=9, height=3)
...
names(mlist)<-c("ZFTA-Cluster 4", "ZFTA-Cluster 2")
