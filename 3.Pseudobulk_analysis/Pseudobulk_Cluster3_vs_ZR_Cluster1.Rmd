...
# Comparison 1: ZFTA-Cluster 3 vs ZFTA-RELA + ZFTA-Cluster 1
```{r subset, message=FALSE, warning=FALSE}
# remove unwanted clusters
Idents(seurat_obj) <- 'Subtype'
seurat_obj <- subset(seurat_obj, idents = c('ST-YAP1', 'ZFTA-Cluster 2', 'ZFTA-Cluster 4'), invert = T)
...
plot_dir <- file.path(base_dir, "analysis/plots/pseudobulk/Cluster3_vs_ZFTARELA_Canonical1")
if (!dir.exists(plot_dir)){dir.create(plot_dir, recursive = T)}
...
markers <- RunPrestoAll(
  ...
  min.pct = 0.15,
  verbose = TRUE)
write.csv(markers, file.path(plot_dir, "1_markers_canonical_vs_noncanonical.csv"))
...
```{r volcano, fig.width =6, fig.height = 4, fig.align="center"}
 ...
 ggplot(markers2, aes(avg_log2FC, -log(p_val_adj,10))) +
   ...
   xlim(c(-6, 6)) +
   labs(color = 'Gene')
  ggsave(file.path(plot_dir, "2_Volcano_plot_cluster3_vs_clu1_ZR.pdf"), width=6, height=4)
...
go_ora_results %>% filter(p.adjust < 0.05) %>%
  ...
  xlab("Gene Ratio") +
  ylab(NULL) + 
  ggtitle("GO Enrichment of Cluster 3")+
...
names(mlist)<-c("ZFTA-RELA/ZFTA-Cluster 1", "ZFTA-Cluster 3")
