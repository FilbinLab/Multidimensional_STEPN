...
breaks <- seq(-120, 370, by= 1)
pdf(file.path(base_dir, 'analysis/neighborhood/all_programs/3_Neighbors_enrichment_average.pdf'), 
    width = 6, height = 4)
Heatmap(as.matrix(nhood_enrichment_zscore), 
        col = colorRamp2(breaks, colorRampPalette(c("steelblue3", 'white', 'red3'))(length(breaks))), 
        cluster_rows = T, 
        cluster_columns = T, 
        show_column_names = F,
...
pdf(file.path(base_dir, 'analysis/neighborhood/all_programs/3_Neighbors_enrichment_average2.pdf'), 
    width = 6, height = 4)
