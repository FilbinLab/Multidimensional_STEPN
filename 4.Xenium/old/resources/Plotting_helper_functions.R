...
colors_sara_clusters <- c('#796409FF', '#B887ADFF', '#FAE9A0FF', '#F5D341FF','#B9E5F3FF', '#689FB0FF', '#FD817EFF', '#FEAB85FF')
...
gg_violinplot_style <- function(df, fill_var) {
  ggplot(df, aes(x=Source_simplified, y=freq, fill = fill_var)) +
    theme(panel.border = element_blank(),
          panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),
          plot.background = element_blank(),
...
gg_violinplot_synaptic <- function(df, fill_var) {
  ggplot(df, aes(x=Source_simplified, y= gene_expression, fill = fill_var)) +
    stat_compare_means(aes(label = after_stat(p.signif)), method = 't.test',size = 3) + NoLegend() +
    theme(panel.border = element_blank(),
          panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),
          plot.background = element_blank(),
          ...
          strip.background = element_rect(fill="grey80")) + scale_y_continuous() 
}
