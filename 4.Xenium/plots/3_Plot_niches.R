...
for (i in seq_along(metadata$Sample)) { 
  ...
  # read data
  data <- qread(file.path(base_dir, paste0('analysis/all_Xenium_runs/data/seurat_objects/seurat_obj_', metadata$SampleID[i], '_', metadata$SampleName[i], '.qs')))
  
  # plot niches
  plots_niches_4[[i]] <- ImageDimPlot(data, group.by = "niches", fov = "fov",  cols = col_niches,
                                      border.size = NA, size = 0.5,
                                      dark.background = F) + NoLegend() +
    labs(title = paste0(metadata$SampleID[i],"_", metadata$SampleName[i]))
  
  # plot niches   k = 5
  plots_niches_5[[i]] <- ImageDimPlot(data, group.by = "niches_k5", fov = "fov",  cols = paletteer::paletteer_d("Redmonder::qPBI"),
                                      border.size = NA, size = 0.5,
                                      dark.background = F) + NoLegend() +
    labs(title = paste0(metadata$SampleID[i],"_", metadata$SampleName[i]))

  # plot niches   k = 6
  plots_niches_6[[i]] <- ImageDimPlot(data, group.by = "niches_k6", fov = "fov",  cols = paletteer::paletteer_d("Redmonder::qMSOPap"),
                                      border.size = NA, size = 0.5,
                                      dark.background = F) + NoLegend() +
    labs(title = paste0(metadata$SampleID[i],"_", metadata$SampleName[i]))
  ...
  # plot groups
  plots_groups[[i]] <- ImageDimPlot(data, group.by = "group", fov = "fov",  cols = colors_metaprograms_Xenium,
                                      border.size = NA, size = 0.5,
                                      dark.background = F) + NoLegend() +
    labs(title = paste0(metadata$SampleID[i],"_", metadata$SampleName[i]))
  
  print(paste0("Finished plotting ", metadata$SampleID[i],"_", metadata$SampleName[i]))
}


# # Plot all tumors together
plot_grid(plotlist = plots_groups, ncol = 5)
ggsave(file.path(plot_dir, paste0('1_ImageDimPlot_metaprogram.pdf')), width=20, height=16)

plot_grid(plotlist = plots_niches_4, ncol = 5)
ggsave(file.path(plot_dir, paste0('2_ImageDimPlot_niches_k4.pdf')), width=20, height=16)

plot_grid(plotlist = plots_niches_5, ncol = 5) 
ggsave(file.path(plot_dir, paste0('2_ImageDimPlot_niches_k5.pdf')), width=20, height=16)

plot_grid(plotlist = plots_niches_6, ncol = 5) 
ggsave(file.path(plot_dir, paste0('2_ImageDimPlot_niches_k6.pdf')), width=20, height=16)
