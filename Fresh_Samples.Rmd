...
``` {r}
data <- readRDS(file.path(base_dir, 'analysis/seurat_obj_malignant.rds'))
...
nmf_score <- nmf_score_final_t %>% 
  dplyr::select(gsub('-| ', '\\.', names(pb_degs)))

set.seed(123456)
tsne <- Rtsne(nmf_score, max_iter = 1000, perplexity = 100, verbose = T)$Y

rownames(tsne) <- rownames(Embeddings(data, "tsne.harmony"))
colnames(tsne) <- c('tSNE_1', 'tSNE_2')
data@reductions$tsne_score <- data@reductions$tsne.harmony
data@reductions$tsne_score@cell.embeddings <- as.matrix(tsne[Cells(data), ])
data@reductions$tsne_score@key <- "tSNE_"

data$Metaprogram<- factor(data$Metaprogram, levels = NMF_order)

p1 <- DimPlot(data,group.by="signature_1",reduction="tsne_score",cols=clusterExperiment::bigPalette,pt.size=1)
p2 <- DimPlot(data,group.by="sample",reduction="tsne_score",pt.size=1)
p1+p2
