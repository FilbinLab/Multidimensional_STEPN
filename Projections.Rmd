...
genes_RadialGlia <- unique(c(ref_degs_1$`Radial glia`, ref_degs_2$`Radial Glia`))
...
#### Running projection for Early development dataset by AgePCW (Sample)
```{r, eval = TRUE, message = FALSE, warning = FALSE}
...
programs_order <- c("Neuroepithelial", "Early Radial glia", "Radial Glia", "MGE Progenitors", "Newborn Excitatory Neurons", "Maturing Excitatory Neurons", "Newborn MGE Neurons", "MGE-derived Interneurons", "CGE-derived Interneurons", "Neuronal", "Choroid", "PCW4", "PCW5", "PCW6", "PCW7", "PCW8")

tab <- rbind(tab_ref_1 %>% mutate(ref = 'Cortex'), tab_ref_2 %>% mutate(ref = 'earlyDev'), tab_ref_3 %>% mutate(ref = 'earlyDev_PCW'))
...
ggsave(plot = p, "/n/scratch/users/c/cao385/Ependymoma/dj83/New/ST_all_hCortex_earlyDev.pdf", width = 10, height = 20, dpi = 1200)
ggsave(plot = p, "/n/scratch/users/c/cao385/Ependymoma/dj83/New/ST_all_hCortex_earlyDev.png", width = 10, height = 20, dpi = 1200)
```
...
ref_degs <- ref_degs[!names(ref_degs) %in% c('Immune', 'Fibroblast', 'Erythrocyte', 'Placodes')]
...
ref_cm <- ref_cm[, !colnames(ref_cm) %in% c('Immune', 'Fibroblast', 'Erythrocyte', 'Placodes')]
ref_degs <- ref_degs[order(match(names(ref_degs),colnames(ref_cm)))]
...
ggsave(plot = pt2, "/n/scratch/users/c/cao385/Ependymoma/dj83/New/ST_samples_Linnarsson_Crest.png", width = 9, height = 18, dpi = 1200)
ggsave(plot = pt2, "/n/scratch/users/c/cao385/Ependymoma/dj83/New/ST_samples_Linnarsson_Crest.pdf", width = 9, height = 18, dpi = 1200)
```
